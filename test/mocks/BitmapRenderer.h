...
class GfxRenderer {
 ...
 private:
  ...
  bool getPixel(int x, int y) const {
    if (x >= 0 && x < SW && y >= 0 && y < SH) return fb[y * SW + x] != 0;
    return false;
  }
      ...
      for (int col = 0; col < 5; col++) {
        if (getGlyphPixel(c, col, row)) {
          for (int sy = 0; sy < scale; sy++)
            for (int sx = 0; sx < scale; sx++) setPixel(x + col * scale + sx, y + row * scale + sy, black);
        }
      }
    }
  }
 ...
 public:
  ...
  void fillRect(int x, int y, int w, int h, bool black = true) {
    for (int py = y; py < y + h; py++)
      for (int px = x; px < x + w; px++) setPixel(px, py, black);
  }
